...
image_paths = glob.glob('/Users/tanayparikh/Desktop/Assignment3 Part-1/A3Part1/*.jpg')
...
n_col = 8
n_row = max(1, int(len(images) / n_col))
f, ax = plt.subplots(n_row, n_col, figsize=(16, 8))
for i in range(n_row):
    for j in range(n_col):
        index = n_col * i + j
        if index < len(images):
            ax[i, j].imshow(list(images.values())[index])
            ax[i, j].set_axis_off()

st.pyplot(f)

st.write("Note: The displayed images are for reference purposes. Upload your image to perform a style-based search.")
